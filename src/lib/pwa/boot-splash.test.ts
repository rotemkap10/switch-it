...
describe("boot splash first-paint helpers", () => {
  ...
  it("skips splash from sessionStorage only outside standalone PWA and native launches", () => {
    ...
    expect(script).toContain("Capacitor");
    expect(script).toContain("isNativePlatform");
  });
});
