...
type AuthCallbackHandlerOptions = {
  /** Dedicated `/auth/callback/recovery` route — never normal app routing. */
  forcePasswordRecovery?: boolean;
};

/**
 * Shared PKCE code exchange for email confirmation and password recovery.
 * Recovery intent must win over onboarding/map/default authenticated redirects.
 */
export async function handleAuthCallback(
  ...
  const recovery = isPasswordRecoveryCallback({
    next,
    type,
    forceRecovery: options.forcePasswordRecovery,
  });
  const authError =
  ...
  if (code) {
    ...
    if (!error) {
      // Recovery sessions are valid but must land on set-new-password first.
      if (recovery) {
