...
| Header credit balance (same `profiles.credits` as Profile) | Implemented |
...
| Live seeker location during active handoff | Implemented (mandatory in active-claim UI; permission/GPS dependent). Web and native both post through Edge Function `handoff-seeker-location`. Web GPS is foreground-only; native can continue in the background. |
| Completion success overlay + return to Find parking | Implemented (overlay lives at app root; `/map` is prepared underneath; MapLibre is not remounted if already on `/map`) |
| Cancel / release / expiry terminal overlay | Implemented (role-specific copy; does not change credit rules) |
...
| Stale-client recovery after a new deploy | Implemented (one hard reload for known chunk/RSC failures; unknown errors still show the error page) |
...
4. Navigate; live location sharing starts for the active claim (if permission/GPS allow). GPS is posted through Edge Function `handoff-seeker-location`.
5. Arrive; publisher confirms; success overlay; Find parking is prepared underneath; credits debit on success.
...
- `src/components/layout/HeaderCreditsBalance.tsx`
- `src/components/handoff/HandoffCompletionSuccessController.tsx`
- `src/components/handoff/HandoffTerminalEndedController.tsx`
- `src/lib/navigation/stale-client-build.ts`
- `src/lib/location/publish-seeker-live-location.ts`
