...
| Edge Function live location | `handoff-seeker-location` — web and native seekers POST GPS here | `supabase/functions/handoff-seeker-location/` |
...
| Capacitor native pilot | Optional native shell + **background** GPS plugin (publish still uses the same Edge Function) | `capacitor.config.ts`, `native/` |
...
flowchart TB
  ...
  Edge[Edge Function handoff-seeker-location]
  Auth[Supabase Auth]
...
App Router handlers: `/auth/callback` and `/auth/callback/recovery`. Live location uses Edge Function `handoff-seeker-location`. Business mutations use **Server Actions**, not a large REST API.
...
| `AppNav` / `HeaderCreditsBalance` | Find parking vs Share a spot; compact header credits from `profiles.credits` |
| `SeekerMapExperience` + MapLibre map | Discovery, selection, claim UI, initial camera / GPS recenter rules |
...
| `HandoffCompletionSuccessController` / `HandoffTerminalEndedController` | App-root overlays; prepare `/map` under the overlay |
| `HistoryList` | Paginated history |
| Live location hooks | Active-claim seeker GPS → Edge Function; publisher Broadcast + snapshot |
| `RouteErrorScreen` | Fatal page + one-time stale-chunk recovery |
...
- Private Broadcast for seeker live location during a claim (published by the Edge Function after DB accept — clients do not publish GPS directly to Realtime).

DB remains authoritative if a message is missed (refresh / reconcile RPCs). Idle / background tabs: Realtime handler throws are contained; disconnect/reconnect must not crash the route. Auth `getSession` / `refreshSession` failures on live-location paths degrade to idle / `unavailable` instead of throwing through the page.

---

# Live location (accurate current behavior)

Web and native seekers use the **same** Edge Function. The difference is how GPS is collected, not how it is published.

| Path | Behavior |
| --- | --- |
| Web / PWA GPS | Foreground `navigator.geolocation.watchPosition` (pauses when hidden) |
| Native GPS | Background plugin during an active handoff |
| Publish | POST `handoff-seeker-location` with seeker JWT + publishable `apikey` |
| Edge Function | `auth.getUser` + `can_send_claim_location` → service-role RPCs → Broadcast **only if accepted** |
| Persistence | Ephemeral **latest** row in `claim_live_locations` (one per claim, replaced on upsert). Deleted when the claim becomes terminal. **Not** a route-history trail |
| Publisher UI | Subscribes to private Broadcast `claim-location:<claimId>`; may **read** the snapshot if the first broadcast was missed |
| Rate limiting | `upsert_claim_live_location` / `try_accept_claim_location_status`: 2s minimum, strict sequence, `claims` row lock |
| Product UX | Sharing is **mandatory** for an active claim in the current UI; still depends on permission/GPS |

Deploying `handoff-seeker-location` is required for **web** live location, not only the native pilot.
...
- Route `error.tsx` / `global-error.tsx` render `RouteErrorScreen`.
- Known stale-build / chunk-load / failed dynamic-import / failed RSC payload errors trigger **one** hard reload (`src/lib/navigation/stale-client-build.ts`), guarded by per-tab `sessionStorage`. Unrelated exceptions do **not** auto-reload.
- Reload on the fatal page always uses `window.location.reload()` (not React `reset()`).
...
- Header credits next to navigation (same balance as Profile).
- Map-first seeker and publisher compose flows.
- After a successful complete: app-root success overlay; `/map` is prepared underneath; overlay dismisses when the seeker map is visually ready (or after a fallback timeout). Already on `/map`: RSC `refresh` only so MapLibre stays mounted.
- After cancel / release / expiry: calmer terminal overlay, then the same return-to-map preparation.
...
- Primary seeker map: **MapLibre + MapTiler** (not Leaflet). Initial camera prefers a trusted GPS fix over stale cache or a Tel Aviv fallback; a late GPS fix does not steal the camera after the user pans.

# PWA / service worker

- Conservative worker: cache version `switch-it-pwa-v12`.
- Precache allowlist: `/offline` and branding/icons only.
- Network-first navigations; offline fallback only when a **navigation** fetch fails.
- Never intercept `/_next/` hashed assets or App Router flight headers (`RSC`, `Next-Router-Prefetch`, `Next-Router-State-Tree`, `Next-Url`).
- `skipWaiting` + `clients.claim`; old `switch-it-pwa-*` caches are deleted on activate.
- Registration uses `updateViaCache: "none"`; not registered inside the native Capacitor shell.
...
- `src/lib/location/use-seeker-live-location-share.ts`, `use-publisher-live-location.ts`, `fetch-claim-live-location.ts`, `publish-seeker-live-location.ts`
- `src/lib/navigation/stale-client-build.ts`
- `src/components/shell/RouteErrorScreen.tsx`
- `src/components/feedback/AppFeedbackRoot.tsx`
- `public/sw.js`
