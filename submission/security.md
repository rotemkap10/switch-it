...
- Idle / resume: token refresh failure on live-location hooks returns `null` / idle and does **not** crash the route. A genuinely missing session is still a login redirect on the next authenticated server render — authentication is not weakened.
...
- Live location is scoped to an **active handoff**:
  - Web and native: POST Edge Function `handoff-seeker-location` with the seeker JWT. The function checks `getUser` + `can_send_claim_location`, then service-role RPCs upsert/throttle, then private Broadcast **only if accepted**.
  - Ephemeral latest row in `claim_live_locations` (one row per claim, replaced; deleted on terminal). **Not** a permanent route history.
  - **Rate limiting:** authenticated seekers cannot flood Realtime with unlimited GPS or status updates.
...
Web and native path: `handoff-seeker-location` Edge Function → service-role RPCs → Realtime Broadcast **only if accepted**. Clients do not publish GPS directly onto Realtime.
