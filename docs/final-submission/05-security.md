...
**Mitigation in this project:** each sensitive RPC checks authentication and role (owner/seeker) before mutating; revoke execute from `anon`/`public` where migrations specify.
...
| Claim spam / griefing | One active claim; distance; credits check | Limited explicit rate limiting | Explicit rate limits |
...
# 12. Future security improvements (NOT CURRENTLY IMPLEMENTED)
